...
const FRAG = /* glsl */ `
  ...
  void main() {
    ...
    for (int k = 0; k < SHEETS; k++) {
      ...
      /* 반폭 — 기본 굵기는 이전(0.21)을 유지하고, 부푸는 상한만 0.40 → 0.29 로 자른다. */
      float hw = mix(0.21, 0.29, r2)
        * (0.92 + 0.16 * (0.5 + 0.5 * fbm1(vec2(u * 1.9 + fk * 3.3 + seed, t * 0.2))));
      ...
      for (int j = 0; j < 7; j++) {
        ...
        float sig = (warm || navy) ? mix(0.042, 0.09, q) : mix(0.008, 0.022, q);
        float ld = (dvo - lc) / sig;
